...
//reverse 브루트포스
int bf_matchr(const char txt[], const char pat[]) {	//it is not perfact
...
int bm_match(const char txt[], const char pat[]) {
	...
	int pat_len = strlen(pat);		//pat 문자개수
	int skip[UCHAR_MAX + 1];		//건너뛰기 표

	for (pt = 0; pt <= UCHAR_MAX; pt++) 	//건너뛰기		UCHAR_MAX 는 unsigned char형 출력이 가능한 문자의 개수
		skip[pt] = pat_len;
